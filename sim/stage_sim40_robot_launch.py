...
def generate_launch_description():
    ...
    declare_world_cmd = DeclareLaunchArgument(
        'world',
        default_value=os.path.join(mwsn_dir, 'maps', 'newsim40v3.world'),
        description='Full path to simulation world file'
    )

    declare_map_file_cmd = DeclareLaunchArgument(
        'map',
        default_value=os.path.join(mwsn_dir, 'maps', 'newsim40v3.yaml'),
    ...
    declare_bag_path_cmd = DeclareLaunchArgument(
        'bag_path',
        default_value=os.path.join(os.path.expanduser( '~' ), 'stage_sim', '40_robot', datetime.now().strftime("%d_%m_%Y_%H:%M:%S")),
    ...
    ld.add_action(
        IncludeLaunchDescription(
                            ...
                            launch_arguments={
                                ...
                                'log_level': 'info',
                                'config': os.path.join(mwsn_dir, 'config', f"parameters_40robot_controller0.yaml"),
    ...
    ld.add_action(
        IncludeLaunchDescription(
                            ...
                            launch_arguments={
                                ...
                                'log_level': 'info',
                                'config': os.path.join(mwsn_dir, 'config', f"parameters_40robot_controller1.yaml"),
                                'localization': 'GroundTruth',
                            }.items())
    )
    ld.add_action(
        IncludeLaunchDescription(
                            ...
                            launch_arguments={
                                ...
                                'log_level': 'info',
                                'config': os.path.join(mwsn_dir, 'config', f"parameters_40robot_controller2.yaml"),
                                'localization': 'GroundTruth',
                            }.items())
    )
    ld.add_action(
        IncludeLaunchDescription(
                            ...
                            launch_arguments={
                                ...
                                'log_level': 'info',
                                'config': os.path.join(mwsn_dir, 'config', f"parameters_40robot_controller3.yaml"),
                                'localization': 'GroundTruth',
                            }.items())
    )
    ...
    instances.append(
        Node(
            ...
            parameters=[
                {'number_of_robots': 40,
                'pose_file': '/home/borja/ros2/ros2_ws/src/ros2_agents/sim/poses_40_robots.yaml',}],
        ))
